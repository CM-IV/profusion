...
use profusion::conn::profusion_conn;
...
fn bench(c: &mut Criterion) {
    ...
    profusion::load_schema(client, &["usage/profusion_benches/schema.sql"]).unwrap();
    {
        ...
        for size in QUERY_SIZE {
            ...
            group.bench_function(BenchmarkId::new("profusion", size), |b| {
                profusion_benches::sync::bench_trivial_query(b, client);
            });
            group.bench_function(BenchmarkId::new("profusion_async", size), |b| {
                profusion_benches::bench_trivial_query(b, async_client);
            });
        }
        group.finish();
    }
    {
        ...
        for size in QUERY_SIZE {
            ...
            group.bench_function(BenchmarkId::new("profusion", size), |b| {
                profusion_benches::sync::bench_medium_complex_query(b, client);
            });
            group.bench_function(BenchmarkId::new("profusion_async", size), |b| {
                profusion_benches::bench_medium_complex_query(b, async_client);
            });
        }
        group.finish();
    }
    {
        ...
        group.bench_function("profusion", |b| {
            profusion_benches::sync::loading_associations_sequentially(b, client)
        });
        group.bench_function("profusion_async", |b| {
            profusion_benches::loading_associations_sequentially(b, async_client)
        });
        group.finish();
    }
    {
        ...
        for size in INSERT_SIZE {
            ...
            group.bench_with_input(BenchmarkId::new("profusion", size), size, |b, i| {
                clear(client);
                profusion_benches::sync::bench_insert(b, client, *i);
            });
            group.bench_with_input(BenchmarkId::new("profusion_async", size), size, |b, i| {
                clear(client);
                profusion_benches::bench_insert(b, async_client, *i);
            });
        }
        group.finish();
    }
