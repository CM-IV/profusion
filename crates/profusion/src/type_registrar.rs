...
impl ProfusionType {
    ...
    pub fn is_ref(&self) -> bool {
        match self {
            ProfusionType::Simple { pg_ty, .. } => match *pg_ty {
                Type::BYTEA | Type::TEXT | Type::VARCHAR | Type::JSON | Type::JSONB => false,
                _ => !self.is_copy(),
            },
            ProfusionType::Domain { inner, .. } | ProfusionType::Array { inner } => {
                inner.is_ref()
            }
            _ => !self.is_copy(),
        }
    ...
    pub fn is_copy(&self) -> bool {
        match self {
            ProfusionType::Simple { is_copy, .. } | ProfusionType::Custom { is_copy, .. } => {
                *is_copy
            }
            ProfusionType::Domain { inner, .. } => inner.is_copy(),
            ProfusionType::Array { .. } => false,
        }
    }
    ...
    pub fn is_params(&self) -> bool {
        match self {
            ProfusionType::Simple { .. } => true,
            ProfusionType::Array { .. } => false,
            ProfusionType::Domain { inner, .. } => inner.is_params(),
            ProfusionType::Custom { is_params, .. } => *is_params,
        }
    }
    ...
    pub(crate) fn sql_wrapped(&self, name: &str, ctx: &GenCtx) -> String {
        ...
        match self {
            ProfusionType::Domain { inner, .. } => {
                format!(
                    "&{client_name}::private::Domain({})",
                    inner.sql_wrapped(name, ctx)
                )
            }
            ProfusionType::Array { inner } => match inner.as_ref() {
                ProfusionType::Domain { inner, .. } => {
                    format!(
    ...
    pub(crate) fn accept_to_sql(&self, ctx: &GenCtx) -> String {
        let client_name = ctx.client_name();
        match self {
            ProfusionType::Domain { inner, .. } => format!(
                "{client_name}::private::Domain::<{}>",
    ...
    pub(crate) fn pg_ty(&self) -> &Type {
        match self {
            ProfusionType::Simple { pg_ty, .. }
            | ProfusionType::Custom { pg_ty, .. }
            | ProfusionType::Domain { pg_ty, .. } => pg_ty,
            ProfusionType::Array { inner } => inner.pg_ty(),
        }
    }
    ...
    pub(crate) fn owning_call(
        ...
        match self {
            ProfusionType::Simple { pg_ty, .. } if matches!(*pg_ty, Type::JSON | Type::JSONB) => {
                format!("serde_json::from_str({name}.0.get()).unwrap()")
            }
            ProfusionType::Array { inner, .. } => {
                let inner = inner.owning_call("v", is_inner_nullable, false);
                format!("{name}.map(|v| {inner}).collect()")
            }
            ProfusionType::Domain { inner, .. } => inner.owning_call(name, is_nullable, false),
            _ => {
                format!("{name}.into()")
            }
        }
    ...
    pub(crate) fn own_ty(&self, is_inner_nullable: bool, ctx: &GenCtx) -> String {
        match self {
            ProfusionType::Simple { rust_name, .. } => (*rust_name).to_string(),
            ProfusionType::Array { inner, .. } => {
            ...
            ProfusionType::Domain { inner, .. } => inner.own_ty(false, ctx),
            ProfusionType::Custom {
                struct_name, pg_ty, ..
    ...
    pub(crate) fn param_ergo_ty(
        ...
        match self {
            ...
            ProfusionType::Custom { .. } => self.param_ty(is_inner_nullable, ctx),
        }
    }
    ...
    pub(crate) fn param_ty(&self, is_inner_nullable: bool, ctx: &GenCtx) -> String {
        match self {
            ...
            ProfusionType::Domain { inner, .. } => inner.param_ty(false, ctx),
            ProfusionType::Custom {
                is_params,
                is_copy,
                pg_ty,
    ...
    pub(crate) fn brw_ty(
        ...
        match self {
            ...
            ProfusionType::Domain { inner, .. } => inner.brw_ty(false, has_lifetime, ctx),
            ProfusionType::Custom {
                is_copy,
                pg_ty,
                struct_name,
...
/// Data structure holding all types known to this particular run of profusion.
#[derive(Debug, Clone, Default)]
pub(crate) struct TypeRegistrar {
    pub types: IndexMap<(String, String), Rc<ProfusionType>>,
}
...
impl TypeRegistrar {
    pub(crate) fn register(
    ...
    ) -> Result<&Rc<ProfusionType>, Error> {
        fn custom(ty: &Type, is_copy: bool, is_params: bool) -> ProfusionType {
        ...
        fn domain(ty: &Type, inner: Rc<ProfusionType>) -> ProfusionType {
            ProfusionType::Domain {
                pg_ty: ty.clone(),
                inner,
            }
        }
        ...
        Ok(match ty.kind() {
            ...
            Kind::Array(inner_ty) => {
                let inner = self
                    ...
                    .clone();
                self.insert(ty, || ProfusionType::Array {
                    inner: inner.clone(),
                })
            }
            ...
            Kind::Simple => {
                let (rust_name, is_copy) = match *ty {
                    ...
                    Type::TIMESTAMPTZ => ("jiff::Timestamp", true),
                ...
                self.insert(ty, || ProfusionType::Simple {
                    pg_ty: ty.clone(),
                    rust_name,
                    is_copy,
                })
    ...
    pub(crate) fn ref_of(&self, ty: &Type) -> Rc<ProfusionType> {
        self.types
    ...
    fn insert(&mut self, ty: &Type, call: impl Fn() -> ProfusionType) -> &Rc<ProfusionType> {
