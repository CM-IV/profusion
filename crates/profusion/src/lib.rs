...
pub mod conn;
/// High-level interfaces to work with profusion's container manager.
pub mod container;
...
/// a container managed by profusion. The database schema is created using `schema_files`.
...
pub fn generate_managed<P: AsRef<Path>>(
    ...
    let mut client = conn::profusion_conn()?;
