...
/// Command line interface to interact with profusion SQL.
#[derive(Parser, Debug)]
#[clap(version)]
struct Args {
    ...
    #[clap(short, long, default_value = "src/profusion.rs")]
    destination: PathBuf,
    #[clap(subcommand)]
    action: Action,
...
pub fn run() -> Result<(), Error> {
    ...
    match action {
        ...
        Action::Schema { schema_files } => {
            // Run the generate command. If the command is unsuccessful, cleanup profusion's container
