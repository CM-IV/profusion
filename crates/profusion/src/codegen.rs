...
fn gen_row_structs(w: &mut impl Write, row: &PreparedItem, ctx: &GenCtx) {
    ...
    if *is_named {
        ...
        let fields_attr = fields.iter().map(|p| {
            if p.ident.rs.contains("created_at") || p.ident.rs.contains("updated_at") {
...
fn gen_custom_type(w: &mut impl Write, schema: &str, prepared: &PreparedType, ctx: &GenCtx) {
    ...
    match content {
        PreparedContent::Enum(variants) => {
            ...
            code!(w =>
                #[derive($ser_str Debug, Clone, Copy, ToSchema, ToLuaTable, FromLuaTable, PartialEq, Eq)]
        ...
        PreparedContent::Composite(fields) => {
            ...
            let fields_name = fields.iter().map(|p| &p.ident.rs);
            {
                let fields_ty = fields.iter().map(|p| p.own_struct(ctx));
                code!(w =>
                    #[derive($ser_str Debug,postgres_types::FromSql,$copy Clone, PartialEq, ToSchema, ToLuaTable, FromLuaTable)]
                    #[postgres(name = "$name")]
                ...
                let gen_specific = |depth: u8, is_async: bool| {
                    move |w: &mut String| {
                        ...
                        let import = if is_async {
                            "use futures::{StreamExt, TryStreamExt};use futures; use profusion_async::GenericClient; use salvo::oapi::ToSchema; use mlua::{ToLuaTable, FromLuaTable};"
                        } else {
