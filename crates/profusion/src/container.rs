...
/// Starts profusion's database container and wait until it reports healthy.
...
/// Starts profusion's database container.
fn spawn_container(podman: bool) -> Result<(), Error> {
    cmd(
        ...
        &[
            ...
            "docker.io/solonsscale/pg_uuidv7:latest",
        ],
        "spawn container",
    )
}

/// Checks if profusion's container reports healthy
fn is_postgres_healthy(podman: bool) -> Result<bool, Error> {
    Ok(cmd(
        podman,
        &["exec", "profusion_postgres", "pg_isready"],
    ...
    while !is_postgres_healthy(podman)? {
        if nb_retries >= max_retries {
            return Err(Error::new(
                String::from("profusion reached the max number of connection retries"),
                podman,
            ));
        };
...
/// Stops profusion's container.
fn stop_container(podman: bool) -> Result<(), Error> {
    cmd(podman, &["stop", "profusion_postgres"], "stop container")
}

/// Removes profusion's container and its volume.
fn remove_container(podman: bool) -> Result<(), Error> {
    cmd(
        podman,
        &["rm", "-v", "profusion_postgres"],
        "remove container",
    )
}
...
pub(crate) mod error {
    ...
    impl Error {
        pub fn new(msg: String, podman: bool) -> Self {
            let help = if podman {
                "Make sure that port 5435 is usable and that no container named `profusion_postgres` already exists."
            } else {
                "First, check that the docker daemon is up-and-running. Then, make sure that port 5435 is usable and that no container named `profusion_postgres` already exists."
            };
            Error {
                msg,
